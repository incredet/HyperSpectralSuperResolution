...
% NMF for Vh 1st (optimised: cache W'*hyper and W'*W when W is fixed)
if strcmp (verbose, 'on'), disp('NMF for Vh (1)'); end
for i = 1:I_in
    if i==1
        cost0 = 0;
        % W is fixed in this branch — precompute once
        WtH_cached = (W_hyper')*hyper;
        WtW_cached = (W_hyper')*W_hyper;
        W_band = W_hyper(1:band,:);
        for q = 1:I_in*3
            % Update H_hyper
            H_hyper_old = H_hyper;
            H_hyper = H_hyper.*WtH_cached./(WtW_cached*H_hyper);
            cost = sum(sum((hyper(1:band,:)-W_band*H_hyper).^2));
...
% NMF for Vm 1st (optimised: cache Gram matrices, avoid full reconstruction for cost)
if strcmp (verbose, 'on'), disp('NMF for Vm (1)'); end
for i = 1:I_in
    if i==1
        cost0 = 0;
        % W is fixed in this branch — precompute once
        WtM_cached = (W_multi')*multi;               % M × 360k — computed once
        WtW_m_cached = (W_multi')*W_multi;            % M × M — tiny
        W_band_m = W_multi(1:multi_band,:);
        % Precompute for fast cost: ||D-WH||^2 = ||D||^2 - 2*sum(W'D .* H) + tr(W'W * HH')
        % where D = multi(1:multi_band,:), W = W_band_m
        WbandtD = W_band_m' * multi(1:multi_band,:); % M × 360k — computed once
        WbandtWband = W_band_m' * W_band_m;          % M × M — tiny
        D_sqnorm = sum(multi(1:multi_band,:).^2, 'all');
        for q = 1:I_in
            % Update H_multi
            H_multi_old = H_multi;
            H_multi = H_multi.*WtM_cached./(WtW_m_cached*H_multi);
            % Fast cost: avoids forming the 11×360k residual matrix
            HHt = H_multi*(H_multi');                 % M × M — tiny
            cost = D_sqnorm - 2*sum(WbandtD.*H_multi, 'all') + sum(WbandtWband.*HHt, 'all');
        ...
        if multi_band > MIN_MS_BANDS
            ...
            W_multi_d = W_multi(1:multi_band,:)*HHt;
