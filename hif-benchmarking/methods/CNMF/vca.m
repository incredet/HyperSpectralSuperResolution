...
fprintf('  VCA: SNR=%.2f  SNRth=%.2f\n', SNR, SNRth);
if (SNR > SNRth)
    fprintf('  VCA: taking HIGH-SNR branch (d=p=%d)\n', p);
    d = p;
    [Ud,~,~] = svds(R*R'/N,d);
    X = Ud'*R;
    u = mean(X, 2);
    denom = sum( X .* repmat(u,[1 N]) );  % (1, N) — dot product per pixel
    fprintf('  VCA: denom range=[%g, %g]  zeros=%d  near-zero(<%g)=%d\n', ...
        min(denom), max(denom), sum(denom==0), 1e-10, sum(abs(denom)<1e-10));
    Y =  X ./ repmat( denom ,[d 1]);
    fprintf('  VCA: Y NaN=%d  Inf=%d\n', sum(isnan(Y(:))), sum(isinf(Y(:))));
else
    fprintf('  VCA: taking LOW-SNR branch (d=p-1=%d)\n', p-1);
    d = p-1;
    r_m = mean(R,2);
...
fprintf('  VCA: U (endmembers) NaN=%d  Inf=%d  range=[%g, %g]\n', ...
    sum(isnan(U(:))), sum(isinf(U(:))), min(U(:)), max(U(:)));
