...
% NMF for Vh (optimised: cache H*H' when H is fixed in i==1 branch)
if strcmp (verbose, 'on'), disp(['NMF for Vh (' num2str(i_out+1) ')']); end
for i = 1:I_in
    if i==1
        cost0 = 0;
        % H is fixed in this branch — precompute once
        HHt_cached = H_hyper*(H_hyper');          % M × M — tiny
        DHt_cached = hyper(1:band,:)*(H_hyper');   % band × M — small
        for q = 1:I_in
            % Update W_hyper
            W_hyper_old = W_hyper;
            W_hyper(1:band,:) = W_hyper(1:band,:).*DHt_cached./(W_hyper(1:band,:)*HHt_cached);
