...
% Collapsed (i,j) into a single parfor for parallelism.
% Each pixel is independent — textbook parfor case.

ntotal = nil * njl;

% Pre-extract per-pixel inputs so parfor can slice them
cell_x = cell(ntotal, 1);
cell_y = cell(ntotal, 1);
cell_f = cell(ntotal, 1);
for idx = 1:ntotal
    i = floor((idx-1) / njl) + 1;
    j = mod(idx-1, njl) + 1;
    imin = (i-1)*mi + 1;  imax = i*mi;
    jmin = (j-1)*mj + 1;  jmax = j*mj;
    cell_x{idx} = reshape(msi(:,jmin:jmax,imin:imax), nm*mi*mj, 1);
    cell_y{idx} = reshape(hsi(:,j,i), nk, 1);
    f_tmp = zeros(nq, mi*mj);
    for q = 1:nq
        f_tmp(q,:) = reshape(fmap(q,jmin:jmax,imin:imax), 1, mi*mj);
    end
    cell_f{idx} = f_tmp;
end

% Parallel MAP estimation over all low-res pixels
cell_z = cell(ntotal, 1);
parfor idx = 1:ntotal
    cell_z{idx} = hre_pixelj_msi(cell_x{idx}, cell_y{idx}, cell_f{idx}, ...
                                  mj_pure, Cj_pure, sigy);
end

% Write results back into output cube
for idx = 1:ntotal
    i = floor((idx-1) / njl) + 1;
    j = mod(idx-1, njl) + 1;
    imin = (i-1)*mi + 1;  imax = i*mi;
