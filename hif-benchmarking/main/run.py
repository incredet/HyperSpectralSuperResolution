...
# Datasets that have no ground-truth (real-world data fed via tif2mat.py).
# Scene discovery uses data/HS/ instead of data/GT/.
NO_GT_DATASETS = [
    "EMIT285",
    "EMIT32",
]
# Datasets with GT for Wald's protocol evaluation.
# Scene discovery uses data/GT/ and metrics can be computed.
WALD_DATASETS = [
    "EMIT32_WALD",
    "EMIT285_WALD",
]
...
SCALES = [4,8,16]
# Per-dataset scale overrides (datasets not listed here use SCALES above)
DATASET_SCALES = {
    "EMIT285":      [6],
    "EMIT32":       [6],
    "EMIT32_WALD":  [6],
...
def main():
    ...
    all_datasets = DATASETS + WALD_DATASETS + NO_GT_DATASETS
    for cd, dataset in enumerate(all_datasets, start=1):
        # Scene discovery: use GT folder when available, otherwise HS folder
        if dataset in NO_GT_DATASETS:
            # For real-world datasets, discover scenes from HS folder (any scale)
            scales = DATASET_SCALES.get(dataset, SCALES)
            img_paths = glob.glob(f'data/HS/{dataset}/{scales[0]}/*.mat')
        else:
            scales = DATASET_SCALES.get(dataset, SCALES)
        ...
        for cm, method in enumerate(methods, start=1):
            for cs, scale in enumerate(scales, start=1):
                for ci, img_path in enumerate(img_paths, start=1):
                    ...
                    print(f"dataset: {dataset} ({cd}/{len(all_datasets)}), method: {method} ({cm}/{len(methods)}), scale: {scale} ({cs}/{len(scales)}), img: {img}({ci}/{len(img_paths)})")
                    ...
                    if not os.path.exists(sri_path):
                        ...
                        if not run_as_papers:
                            ...
                            if dataset in NO_GT_DATASETS:
                                print(f"  SKIP: --paper mode not supported for {dataset} (no GT)")
                                continue
