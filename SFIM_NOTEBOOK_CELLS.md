# SFIM Fusion — Colab Notebook

Minimal notebook that calls `spectral/run_sfim.py`. All algorithm
...
Calls `spectral/run_sfim.py` to run SFIM hypersharpening fusion on QC-clean tiles.

**Inputs**: `tiles_clean.csv` (from `Tile_QC.ipynb`), EMIT-b32 + S2 tile pairs
**Outputs**: fused GeoTIFFs in `{tiles_dir}/`, per-tile R² CSV
```
...
```python
!pip install -q numpy scipy scikit-learn rasterio matplotlib tqdm joblib opencv-python-headless earthaccess pyyaml
```
...
## Cell 5 — Run SFIM (parallel variant — copy notebook, change start/stop)
...
df = pd.read_csv(drive_base / "r2_sfim_tiles.csv")
ok = df[df["status"] == "OK"]

print(f"Total: {len(df)}, OK: {len(ok)}")
print(f"R² mean={ok['r2_sfim_mean'].mean():.4f}, "
